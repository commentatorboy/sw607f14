...
\end{longtable}
Changing the name of a sequence was previously done by clicking on the title text, but it was not obvious that it was in fact editable.  The issue was solved by adding a button (as shown in figure \ref{fig:Old_editName}) which programatically places the cursor in the textfield and popping up the virtual keyboard. We felt like this cluttered the entire UI a little bit with too many buttons, so we decided to keep the issue open for later revising. As there no existing icons to represent editing the name, we used a simple white placeholder with the text `TEXT' on it. 
\begin{figure} [h!]
\centering
